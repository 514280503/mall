...
<div id="list">
	<table>
		...
		<tr><td>{$value->user}</td><td>{$value->level_name}</td><td>{$value->login_count}</td><td>{$value->last_ip}</td><td>{$value->last_time}</td><td><a href="?a=manage&m=update&id={$value->id}"><img src="view/admin/images/edit.gif" alt="编辑" title="编辑" /></a> <a href="?a=manage&m=delete&id={$value->id}" onclick="return confirm('确认删除这个管理员吗？') ? true : false"><img src="view/admin/images/drop.gif" alt="删除" title="删除" /></a></td></tr>
		{foreachelse}
