...
class NavAction extends Action {
	...
	public function add() {
		if (isset($_POST['send'])) $this->_model->add() ? $this->_redirect->succ('?a=nav', '分类新增成功！') : $this->_redirect->error('分类新增失败！');
        ...
        $this->_tpl->assign('AllPrice', Tool::setFormItem($this->_price->findNavPrice(), 'value', 'value'));
		$this->_tpl->display(SMARTY_ADMIN.'nav/add.tpl');
	}
	
	public function update() {
		if (isset($_POST['send'])) $this->_model->update() ? $this->_redirect->succ(Tool::getPrevPage(), '分类名称修改成功！') : $this->_redirect->error('分类名称修改失败！');
		if (isset($_GET['id'])) {
	...
	public function delete() {
		if (isset($_GET['id'])) $this->_model->delete() ? $this->_redirect->succ(Tool::getPrevPage(), '分类名称删除成功！') : $this->_redirect->error('分类名称删除失败！');
	}
